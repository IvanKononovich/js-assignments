...
function getRectangleString(width, height) {
    ...
    for (let i = 0; i < height; i++) {
        for (let j = 0; j < width; j++) {
            if(i === 0 && j === 0) result.push('┌');
            if(i === 0 && j === width - 1) result.push('┐');
            if(i === height - 1 && j === 0) result.push('└');
            if(i === height - 1 && j === width - 1) result.push('┘');
            if((i !== 0 && i !== height - 1) && (j !== 0 && j !== width - 1)) result.push(' ');
            if((i !== 0 && i !== height - 1) && (j === 0 || j === width - 1)) result.push('│');
            if((i === 0 || i === height - 1) && (j !== 0 && j !== width - 1)) result.push('─');
            if(j === width - 1) result.push('\n');
        }
    }
 ...
 *   'ABCDEFGHIJKLMNOPQRSTUVWXYZ' => 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'
 *
 */
function encodeToRot13(str) {
    ...
    for (let i = 0; i < alf.length / 2; i++) {
        cipherTable[alf[i]] = alf[i + 13];
        cipherTable[alf[i + 13]] = alf[i];
        cipherTable[alf[i].toUpperCase()] = alf[i + 13].toUpperCase();
        cipherTable[alf[i + 13].toUpperCase()] = alf[i].toUpperCase();
    }

    return str.split('').map(item => {
        if(/\w/.test(item)) item = cipherTable[item];
        return item;
    }).join('');
}
